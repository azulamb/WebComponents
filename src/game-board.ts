...
((script, init) => {
  ...
  ((component, tagname = 'game-board') => {
    ...
    class extends HTMLElement implements GameBoardElement {
      ...
      // 色の名前と初期値
      protected colors: { name: string; var: string; color: string }[] = [{
        name: 'on',
        var: 'select',
        color: 'rgba(137, 157, 255, 0.5)',
      }];
      constructor() {
        super();
      ...
      private updateBoard() {
        ...
        styles.push(
          ...
          ':host > div.main { padding: calc( 100% * ' + height + ' / ' + width +
            ' ) 0 0; }',
          'div.board { ' +
            ...
            ' }',
          // 駒の大きさを指定しておきます。
          '::slotted( :not[ slot ] ) { width: calc( 100% / ' + width +
        ...
        this.colors.forEach((color) => {
          styles.push(
            'div.board > div.' + color.name + ' { background-color: var( ' +
      ...
      protected addPieceStyle(
        ...
        styles.push(
          'div.board > [ data-position = "' + position + '" ] {' +
            'grid-column: ' + x + '/' + (x + 1) + ';grid-row: ' + y + '/' +
            (y + 1) + ';' +
            '}',
          '::slotted( [ data-position = "' + position +
            '" ] ) { left: calc( ( 100% ' + ' * ' +
            (x - 1) + ' ) / ' + this.width + ' ); top: calc( 100% ' + ' * ' +
            (y - 1) + ' / ' + this.height + ' ); }',
        );
      }
      ...
      public select(x: number, y: number, ...colors: string[]) {
        ...
        const box = this.board.querySelector(
      ...
      private onUpdateWidth(value: string | number) {
        // 値チェック
        if (
          this.width !== (typeof value === 'number' ? value : parseInt(value))
        ) {
      ...
      private onUpdateHeight(value: string | number) {
        // 値チェック
        if (
          this.height !== (typeof value === 'number' ? value : parseInt(value))
        ) {
