...
interface InputSliderElement extends HTMLElement {
	min: number;
	max: number;
	step: number;
	value: number;
		...
		class extends HTMLElement implements InputSliderElement {
			...
			constructor() {
				...
				if (this.hasAttribute('min')) {
					this.input.min = <string> this.getAttribute('min');
					this.slider.min = this.input.min;
				}

				if (this.hasAttribute('max')) {
					this.input.max = <string> this.getAttribute('max');
					this.slider.max = this.input.max;
				}

				if (this.hasAttribute('step')) {
					this.input.step = <string> this.getAttribute('step');
					this.slider.step = this.input.step;
				}

				if (this.hasAttribute('value')) {
					this.input.value = <any> this.getAttribute('value');
				...
				(() => {
					...
					this.slider.addEventListener('change', () => {
						this.value = <any> this.slider.value;
						onChange();
					});
					this.input.addEventListener('change', () => {
						this.value = <any> this.input.value;
						onChange();
					});
				})();
