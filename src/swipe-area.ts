    ...
    class extends HTMLElement implements SwipeAreaElement {
      ...
      constructor() {
        ...
        style.textContent = [
          ...
          ':host( [ disabled ] ) > div { pointer-events: none; }',
        ...
        let touched = false;
        contents.addEventListener('touchstart', (event) => {
          touched = true;
          this.begin(event.touches[0].clientX, event.touches[0].clientY);
        });
        contents.addEventListener('touchmove', (event) => {
          this.move(event.touches[0].clientX, event.touches[0].clientY);
        });
        contents.addEventListener('touchend', (event) => {
          this.end();
        });
        contents.addEventListener('touchcancel', (event) => {
          this.cancel();
        });
        ...
        contents.addEventListener('mouseup', (event) => {
          if (!touched) {
            this.end();
          }
      ...
      private end() {
        if (
          Number.isNaN(this.sx) || Number.isNaN(this.sy) ||
          Number.isNaN(this.ex) || Number.isNaN(this.ey)
        ) {
          return;
        }
        ...
        if (
          (this.ex - this.sx) ** 2 + (this.ey - this.sy) ** 2 <
            this.distance ** 2
        ) {
          return;
        }
        this.dispatchEvent(
          new CustomEvent<SwipeAreaData>('swipe', {
            detail: {
              sx: this.sx,
              sy: this.sy,
              ex: this.ex,
      ...
      get distance() {
        return parseInt(this.getAttribute('distance') || '') || 30;
      }
      set distance(value) {
